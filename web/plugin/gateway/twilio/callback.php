...
$remote_smslog_id = $_REQUEST['SmsSid'];
...
if ($remote_smslog_id && $status && ($status != 'received')) {
	...
	if ($smslog_id) {
		$data = sendsms_get_sms($smslog_id)[0];
		$uid = $data['uid'];
...
$sms_sender = $_REQUEST['From'];
$message = htmlspecialchars_decode(urldecode($_REQUEST['Body']));
$sms_receiver = $_REQUEST['To'];
$smsc = $_REQUEST['smsc'];
